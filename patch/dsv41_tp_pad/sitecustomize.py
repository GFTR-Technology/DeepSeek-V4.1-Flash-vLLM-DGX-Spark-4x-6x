"""Injected into every rank via ``PYTHONPATH=/opt/dsv41``.
...
register a post-import hook that calls into ``dsv41_tp_pad`` once the relevant
...
_ENABLED = os.environ.get("DSV41_TP_PAD", "").strip() not in ("", "0", "1")
_PREFIX = "[dsv41-tp-pad]"
...
def _chain_previous_sitecustomize():
    ...
    for entry in sys.path:
        ...
        spec = importlib.util.spec_from_file_location(
            "_dsv41_chained_sitecustomize", candidate
        )
...
def _fire(module_name, module):
    ...
    hook = dsv41_tp_pad.HOOKS.get(module_name)
    if hook is None:
        return
    try:
        hook(module)
        ...
        print(f"{_PREFIX} FATAL: patching {module_name} failed", file=sys.stderr)
        raise
...
def _install():
    try:
        ...
        print(
            f"{_PREFIX} FATAL: DSV41_TP_PAD is set but dsv41_tp_pad is not "
            f"importable from PYTHONPATH ({exc})",
            file=sys.stderr,
        )
        raise
    sys.meta_path.insert(0, _PostImportHook(dsv41_tp_pad.HOOKS))
    # Covers the case where vLLM was somehow already imported.
    dsv41_tp_pad.install()
