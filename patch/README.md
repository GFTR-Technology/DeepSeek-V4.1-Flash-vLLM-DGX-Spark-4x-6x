...
|---|---|---|---|
| `engram.py` | 41f16f34 | `models/deepseek_v4_1/common/engram.py` | Engram tables on disk, rank-offset fix, one shared parallel read pool, `EngramDiskStager`, optional node-local rows (`DSV41_ENGRAM_DIR`, used only when the copied row range covers the rank), zero-column ranks under a TP that does not divide `n_hash_cols` |
...
| `dsv41_tp_pad/` | TP alignment by padding, for a TP that does not divide the model (TP=6). Not bind-mounted over vLLM: a `sitecustomize.py` shim on `PYTHONPATH` plus a config overlay. Tests in `tests/test_tp_pad.py`. |
