...
> **This page describes the original launch path** (`launch/dsv41-tp4.sh <rank>`,
> `launch/boot_dsv41.sh`, `~/patches/$PATCH_NAME`), which is what every measured
> number in the [README](../README.md) was produced on and which still works.
>
> The current path is `scripts/`: one `cluster.env`, then
> `./scripts/dsv41-serve.sh`. It does the same thing with dual-rail RoCE, the
> patches mounted straight from the repo clone, node-local Engram rows on by
> default, and padding for a TP that does not divide the model. Steps 0-4 and 6-7
> below (hardware, download, NFS, images, patches, verification, host hardening)
> apply to both; step 5 is the part `scripts/dsv41-serve.sh` replaces.
> See [README_CN.md](../README_CN.md) for the current end-to-end walkthrough.
